...
1. `go2 backlog` — ready tickets, highest priority first. If an id was given,
   use that one; it must be `ready` (or `in-progress` with your own name as
   owner, if resuming).
...
5. `go2 backlog index && go2 backlog check`.
...
3. `go2 backlog index && go2 backlog check`.
